  ...
  for (const link of links) {
    ...
    link.addEventListener("click", (ev) => {
      ...
      try {
        ...
        // Fallback for cores where SettingsConfig must be instantiated
        const SettingsApp =
          globalThis.SettingsConfig ??
          (globalThis.foundry?.applications?.config?.SettingsConfig);
        if (SettingsApp) {
          new SettingsApp().render(true);
          return;
        }
        ui.notifications?.warn("Unable to open Settings automatically. Please click the gear icon in the top-right.");
      } catch (err) {
        console.error(`[${MODULE_ID}] Failed to open Settings:`, err);
